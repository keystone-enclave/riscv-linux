...
static int ovl_get_upper(struct ovl_fs *ofs, struct path *upperpath)
{
	...
	ofs->upper_mnt = clone_private_mount(upperpath);
	err = PTR_ERR(ofs->upper_mnt);
	if (IS_ERR(ofs->upper_mnt)) {
		pr_err("overlayfs: failed to clone upperpath\n");
		goto out;
	}

	/* Don't inherit atime flags */
	ofs->upper_mnt->mnt_flags &= ~(MNT_NOATIME | MNT_NODIRATIME | MNT_RELATIME);
	err = 0;
out:
	return err;
}
